...
export class ProjectRepository extends BaseRepository<Project> {
  ...
  /**
   * Find all projects for a user with task counts in a single query.
   */
  findByUserIdWithTaskCounts(userId: string, options: FindProjectsOptions = {}): ProjectWithTaskCount[] {
    const { status, search, includeArchived = false } = options;
    const clauses: string[] = ['p.user_id = ?'];
    const params: unknown[] = [userId];
    ...
    return this.query<ProjectWithTaskCount>(sql, params);
  }
